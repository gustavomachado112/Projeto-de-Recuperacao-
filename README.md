...
Link do GitHub para os códigos Python:
https://github.com/gustavomachado112/Projeto-de-Recuperacao-/tree/main
...
Agora são específicas e detalhadas, incluindo previsão de tempo de resposta.CONFIG.PY
...
Esta correção elimina completamente os erros de execução relacionados à conversão de dados, preserva zeros iniciais em telefones, mantém o formato visual correto das datas e garante que os dados sejam armazenados exatamente como o usuário forneceu.
...
Estas validações garantem a integridade e qualidade dos dados cadastrados, previnem erros futuros no sistema, orientam os usuários a fornecer informações corretas e melhoram significativamente a experiência do usuário com feedbacks claros.
...
Protege a privacidade e segurança do usuário, previne que terceiros visualizem a senha, atende aos padrões básicos de segurança de sistemas e cria uma barreira importante contra a captura não autorizada de credenciais.
...
Melhora drasticamente a experiência do usuário com comunicação clara e objetiva, reduz tentativas frustradas, diminui a taxa de abandono do sistema e educa o usuário sobre os formatos corretos de preenchimento.
...
A minha entidade doacao_tb inicialmente continha o campo id_conta_orig.

No entanto, a conta de origem de uma doação já podia ser identificada através da campanha_tb, que possui o id_conta_orig.

Manter esse campo diretamente em doacao_tb gerava redundância.
...
Na entidade pagamento_tb, havia os campos id_conta e nome.

Como o id_doacao já identifica a doação, e por sua vez, a doação está associada a uma campanha e a uma conta (via doacao_tb -> campanha_tb -> conta_orig_tb e doacao_tb -> id_conta), esses campos eram redundantes.

O pagamento está diretamente vinculado a uma doação, então armazenar id_conta e nome era desnecessário.
...
As entidades contato_tb e contato_para_conta_org_tb tinham funções muito semelhantes, com campos como id_contato, nome, email e mensagem se repetindo.

Isso era um forte indício de que a informação estava sendo armazenada em mais de um lugar, o que era um erro de normalização.
...
Protege efetivamente contra ataques de força bruta, adiciona uma camada importante de segurança ao sistema, segue as boas práticas de segurança reconhecidas internacionalmente e protege as contas dos usuários contra acessos não autorizados.
